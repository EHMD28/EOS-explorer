...
def draw_info_for_tabulated_eos():
    ...
    st.markdown("""The values used to generate the mass-radius curve are
                interpolated directly from a tabulated EoS file. See
                [the files in this repo](https://github.com/EHMD28/EOS-explorer/tree/ece583b8a5564f16a35920c699cc5da33d771d3e/data/eos/mev)
                for the expected format. Note that the application infers to
                delimeter based on the file extension (see the Info tab for more
                details)""")


def plot_tabulated_eos(ax: Axes):
    eos_file = st.session_state[StreamlitKeys.EOS_FILE_UPLOAD_WIDGET]
    if eos_file is not None:
        name, ext = eos_file.name.split(".")
        eos_data = load_eos_from_file(eos_file, extension=ext)
        if eos_data is not None:
            densities, pressures = eos_data
            ax.scatter(densities, pressures, label=name)
        return eos_data if eos_data is not None else None
...
def draw_mr_file_upload_widget():
    ...
    st.markdown(
        ...
        " unused). Any file generated by EoS Explorer is compatible. The"
        " delimeter of the data file is inferred from the file extension (see"
        " the Info tab for more details)."
    )
    st.file_uploader(
...
def plot_uploaded_mr_curves(ax: Axes):
    ...
    for file in mr_files:
        ...
        data = load_mr_curve_from_file(file, ext)
        if data is not None:
            radii, masses = data
